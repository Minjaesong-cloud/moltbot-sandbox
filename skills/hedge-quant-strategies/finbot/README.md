...
## 3. `kr_stock_engine.py` — 한국 종목 듀얼 모멘텀 (보텀업·한국)
...
## 5. `candidate_report.py` — 통합 후보 리포트

스크리너 상위 후보를 Damodaran 산업 평균과 **자동 대조**(SIC→산업 키워드 매핑): 시총 E/P vs
산업 E/P, ROE vs 산업 ROE, EP격차 컬럼. "산업보다 싼가 → 왜 싼가(ROE로 정당한 할인인가)"를
표 하나로. 파이프라인 ③단계(PIPELINE-DEMO.md에선 수동)의 자동화.
